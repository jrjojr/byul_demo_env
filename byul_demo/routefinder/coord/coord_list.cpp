...
coord_t* coord_list_pop_back(coord_list_t* list) {
    if (list && !list->data.empty()) {
        coord_t* c = coord_copy(&list->data.back());
        list->data.pop_back();
        return c;  // 호출자가 coord_free() 해야 함
    }
    return nullptr;
}
...
coord_t* coord_list_pop_front(coord_list_t* list) {
    if (list && !list->data.empty()) {
        coord_t* c = coord_copy(&list->data.front());
        list->data.erase(list->data.begin());  // 앞 요소 제거
        return c;  // 호출자가 coord_free() 해야 함
    }
    return nullptr;
}
...
coord_list_t* coord_list_sublist(
    ...
    sub->data.assign(list->data.begin() + start, list->data.begin() + end);
    return sub;
}
