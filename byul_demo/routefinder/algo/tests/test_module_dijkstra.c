...
static void test_dijkstra_simple_route(void) {
    coord start = coord_new_full(0, 0);
    coord goal = coord_new_full(9, 9);

    // algo al = algo_new();
    algo al = algo_new_full(10, 10, MAP_NEIGHBOR_8, PATH_ALGO_DIJKSTRA,
        default_cost,
        default_heuristic,
        NULL,
        NULL,
...
static void test_dijkstra_blocked_route(void) {
    coord start = coord_new_full(0, 0);
    coord goal = coord_new_full(9, 9);
...
int run_dijkstra_tests(void) {
    g_test_add_func("/dijkstra/simple_route", test_dijkstra_simple_route);
    g_test_add_func("/dijkstra/blocked_route", test_dijkstra_blocked_route);
    return 0;
}
