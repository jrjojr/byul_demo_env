...
struct s_dstar_lite_pqueue {
    std::unordered_map<dstar_lite_key_t*, std::vector<coord_t*>,
        DstarLiteKeyHash, DstarLiteKeyEqual> key_to_coords;
...
void dstar_lite_pqueue_free(dstar_lite_pqueue_t* q) {
    ...
    // key_to_coords의 coord_t*만 해제 (key는 공유)
    for (auto& [key, coord_vec] : q->key_to_coords) {
        for (coord_t* c : coord_vec) {
            coord_free(c);
        }
        dstar_lite_key_free(key);
    }
    // q->key_to_coords.clear();

    // key 소유권은 coord_to_key가 가지므로 여기서 free
...
dstar_lite_pqueue_t* dstar_lite_pqueue_copy(const dstar_lite_pqueue_t* src) {
    ...
    for (const auto& [key, coords] : src->key_to_coords) {
        ...
        copy->key_to_coords[key] = std::move(copied_coords); // key는 공유
    }
...
void dstar_lite_pqueue_push(dstar_lite_pqueue_t* q,
    ...
    dstar_lite_key_t* old_key = const_cast<dstar_lite_key_t*>(key);
    auto it = q->key_to_coords.find(old_key);

    if (it != q->key_to_coords.end()) {
        // old_key가 존재한다. 해당 vector에 c 추가
        it->second.push_back(coord_copy(c));
    } else {
        // old_key가 존재하지 않음, 새로 추가
    ...
    // coord_to_key 업데이트
    coord_hash_replace(q->coord_to_key, c, (void*)key);
}
...
bool dstar_lite_pqueue_remove(dstar_lite_pqueue_t* q, const coord_t* u) {
    ...
    auto* key_ptr = static_cast<dstar_lite_key_t*>(
