...
TEST_CASE("map clear all") {
    ...
    CHECK(map_is_blocked(m, 2, 2));
    map_clear(m);
    CHECK_FALSE(map_is_blocked(m, 2, 2));
    map_free(m);
}
...
TEST_CASE("map neighbors filtering") {
    ...
    coord_list_t* neighbors = map_clone_neighbors(m, 2, 2);
    REQUIRE(neighbors);
...
TEST_CASE("map neighbor at degree") {
    ...
    coord_t* c = map_clone_neighbor_at_degree(m, 2, 2, 0.0);
    REQUIRE(c);
...
TEST_CASE("map neighbor at goal") {
    ...
    coord_t* c = map_clone_neighbor_at_goal(m, center, goal);
    REQUIRE(c);
    CHECK(coord_get_x(c) == 3);
...
TEST_CASE("map cone neighbor range") {
    ...
    coord_list_t* result = map_clone_neighbors_at_degree_range(
